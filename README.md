...
複数の物理チェーン（髪、スカートなど）の剛体を選択し、質量とスプリングを一括で良い感じに設定するプラグインです。
使い方は`物理チェーン自動調整プラグイン.txt`にて。

### CSV物理パラメータ適用プラグイン

PMX Editor で出力した剛体・ジョイントのCSVファイルを指定して、そのパラメータを現在のモデルを対象に設定するプラグインです。
モデル側にすでに存在するもののみが対象となります。
使い方は`CSV物理パラメータ適用プラグイン.txt`にて。
